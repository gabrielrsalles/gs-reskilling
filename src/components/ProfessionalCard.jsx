export default function ProfessionalCard({ profissional, onClick }) {
    ...
    return (
      <button onClick={onClick} className="group flex flex-col rounded-xl border border-green-600/40 bg-green-50/60 p-4 text-left shadow-lg transition-transform hover:-translate-y-1 hover:shadow-2xl focus:outline-none focus:ring-2 focus:ring-green-500/60" >
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          ...
          {habilidadesTecnicas.length > 3 && ( <span className="inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-[10px] text-green-600 dark:bg-green-900/40 dark:text-green-200"> +{habilidadesTecnicas.length - 3} </span> )}
        </div>
